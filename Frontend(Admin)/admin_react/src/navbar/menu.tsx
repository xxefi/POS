...
import { menusItem, MenusItem } from "../components/interfaces/menus.item";
import { activeStyle, buttonStyle } from "../components/styles/button.styles";

export default function Menu({ t, setOpen, open }: ComponentProps) {
  const renderedMenusItem = (item: MenusItem) => (
    <li key={item.to}>
      <Button
        component={NavLink}
        to={item.to}
    ...
    <li className="mb-2">
      ...
      >
        <span className="flex items-center">
          <FoodBankOutlined className="mr-2" />
          {t("menu")}
        </span>
        <svg
      ...
      >
        ...
        >
          {menusItem.map(renderedMenusItem)}
        </ul>
      </div>
    </li>
  );
