...
import {
  financesItem,
  FinancesItem,
} from "../components/interfaces/finances.item";
import { activeStyle, buttonStyle } from "../components/styles/button.styles";

export default function Finances({ t, setOpen, open }: ComponentProps) {
  const renderedFinanceItem = (item: FinancesItem) => (
    <li key={item.to}>
      <Button
        component={NavLink}
        to={item.to}
    ...
    <li className="mb-2">
      ...
      >
        <span className="flex items-center">
          <AttachMoneyRounded className="mr-2" />
          {t("finances")}
        </span>
        <svg
      ...
      >
        ...
        >
          {financesItem.map(renderedFinanceItem)}
        </ul>
      </div>
    </li>
  );
