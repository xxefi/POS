import { SaleMethodData } from "../../../interfaces/component.types.ts";
import { useTranslation } from "react-i18next";
import CheckCircleIcon from "@mui/icons-material/CheckCircle";
import CancelIcon from "@mui/icons-material/Cancel";

interface SaleMethodTableProps {
  saleMethod: SaleMethodData[];
}

export default function SaleMethodTable({ saleMethod }: SaleMethodTableProps) {
      ...
      >
        ...
        <tbody>
          {saleMethod.length === 0 ? (
            ...
            saleMethod.map((saleMethod, index) => (
              <tr
                key={saleMethod.id}
              ...
              >
                <td className="py-3 px-4 rounded-l-lg">
                  <span>{saleMethod.id}</span>
                </td>
                <td className="py-3 px-4">
                  <span>{saleMethod.name}</span>
                </td>
                <td className="py-3 px-4">
                  <span>{saleMethod.paymentMethod}</span>
                </td>
                <td className="py-3 px-4 rounded-r-lg">
                  {renderIcon(saleMethod.status)}
                </td>
              </tr>
            ))
          )}
